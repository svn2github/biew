...
void HexMode::misckey_action () /* EditHex */
{
	...
	while(1) {
	    ...
	    if(!active) _lastbyte = full_hex_edit(MainWnd);
	    else        _lastbyte = FullEdit(MainWnd,*this,NULL);
...
bool HexMode::action_F2() /* hexSelectMode */
{
    ...
    retval = SelBoxA(const_cast<char**>(names),nModes," Select hexadecimal mode: ",hmode);
    if(retval != -1) {
	hmode = retval;
	check_width_corr();
	return true;
...
bool HexMode::action_F3 () /* hexSelectEndian */
{
    ...
    retval = SelBoxA(const_cast<char**>(nendian),nModes," Select endian mode: ",hendian);
    if(retval != -1) {
	hendian = retval;
	return true;
    }
...
bool hexAddressResolution(unsigned& har)
{
    ...
    i = SelBoxA(const_cast<char**>(aresolv),nModes," Select address resolving: ",(unsigned)har);
    if(i != -1) {
	har = i ? true : false;
	return true;
    }
...
int HexMode::full_hex_edit(TWindow * txtwnd)
{
    size_t i,j;
    unsigned mlen;

...
#define twrite(x)	write(out_fd, (x), strlen((char *)x))
...
*/
	    ...
	    {
		unsigned char *ptr;
		ptr = (unsigned char*)_2ansi(ca);
		strcpy((char*)dpb, (char*)ptr);
		dpb += strlen((char*)ptr);
	    }
	    if(is_unicode) {
		unsigned _len=1;
		char *destb=nls_recode2screen_cp(nls_handle,(char *)&c,&_len);
...
void __FASTCALL__ __init_vio(const char *user_cp,unsigned long flags)
{
    ...
    if (on_console) {
	unsigned char *buf = new unsigned char [violen << 1];
	...
	if (terminal->type != TERM_LINUX) {
		...
		twrite("\033[?1000h\033]0;BEYE: Binary EYEer\007");
		frames_vt100[0x2B] =
		frames_vt100[0x2C] =
		frames_vt100[0x2D] =
		frames_vt100[0x2E] =

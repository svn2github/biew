...
{
  close(reinterpret_cast<long>(handle));
}

bhandle_t __FASTCALL__ __OsDupHandle(bhandle_t handle)
{
  return (bhandle_t*)dup(reinterpret_cast<long>(handle));
}
...
{
  return lseek(reinterpret_cast<long>(handle),offset,origin);
}

int __FASTCALL__ __OsTruncFile(bhandle_t handle, __filesize_t newsize)
{
  return ftruncate(reinterpret_cast<long>(handle),newsize);
}

int __FASTCALL__ __OsRead(bhandle_t handle, any_t*buff, unsigned count)
{
  return read(reinterpret_cast<long>(handle),buff,count);
}

int __FASTCALL__ __OsWrite(bhandle_t handle,const any_t*buffer, unsigned count)
{
  return write(reinterpret_cast<long>(handle),buffer,count);
}
...
int __FASTCALL__ __OsChSize(bhandle_t handle, __fileoff_t size)
{
    ...
    bufsize= (int)std::min(fillsize, __fileoff_t(BLKSIZE));
    buf=new char[bufsize];
    if(buf == NULL) return -1;
    memset(buf, 0, bufsize);		    /* write zeros to pad file */
    do
    {
	numtowrite= (int) std::min(fillsize, (long) bufsize);
...
__fileoff_t __FASTCALL__ __FileLength(bhandle_t handle)
{
  ...
  stat_ret = fstat(reinterpret_cast<long>(handle),&statbuf);
  retval = 0;
  if(stat_ret == 0)
  {
  ...
  if(!retval)
  {
	...
	{
	   off = min + ((max - min) / 2);
	   if (lseek(reinterpret_cast<long>(handle),off,SEEK_SET) == -1)
		max=off;
	   else {
	   if(read(reinterpret_cast<long>(handle),&ch,1) == 1) min = off;
	   else                        max = off;
	   }
	   if(max - min < 5)
	   {
	     retval = min;
	     lseek(reinterpret_cast<long>(handle),retval,SEEK_SET);
	     for(i=0;i < 5;i++)
	     {
		if(read(reinterpret_cast<long>(handle),&ch,1) == 1) retval++;
		else goto loop_end;

/**
 * @namespace   libbeye
 * @file        libbeye/sysdep/ia16/os2/os_dep.c
...
{
  USHORT rc;
  rc = DosCreateSem(CSEM_PUBLIC,&beyeSem,(PSZ)"\\SEM\\BEYE.SEM");
  if(rc == ERROR_ALREADY_EXISTS) /* means still one copy of beye already runing */
  {
     rc = DosOpenSem(&beyeSem,(PSZ)"\\SEM\\BEYE.SEM");
     if(rc)
     {
